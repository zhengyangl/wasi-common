...
pub(crate) unsafe fn fd_datasync(wasi_ctx: &WasiCtx, fd: wasi::__wasi_fd_t) -> Result<()> {
    ...
    let fd = dec_fd(fd);
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_DATASYNC, 0)
...
pub(crate) unsafe fn fd_pread(
    ...
    let fd = dec_fd(fd);
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_READ, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn fd_read(
    ...
    let fe = wasi_ctx.get_fd_entry_mut(fd, wasi::__WASI_RIGHT_FD_READ, 0)?;
...
pub(crate) unsafe fn fd_tell(
    ...
    let fd = dec_fd(fd);
    let fd = wasi_ctx
        .get_fd_entry_mut(fd, wasi::__WASI_RIGHT_FD_TELL, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file_mut())?;
...
pub(crate) unsafe fn fd_sync(wasi_ctx: &WasiCtx, fd: wasi::__wasi_fd_t) -> Result<()> {
    ...
    let fd = dec_fd(fd);
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_SYNC, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn fd_write(
    ...
    let fd = dec_fd(fd);
    let iovs = dec_iovec_slice(memory, iovs_ptr, iovs_len)?;
    let fe = wasi_ctx.get_fd_entry_mut(fd, wasi::__WASI_RIGHT_FD_WRITE, 0)?;
...
pub(crate) unsafe fn fd_advise(
    ...
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_ADVISE, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn fd_allocate(
    ...
    let len = dec_filesize(len);
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_ALLOCATE, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn path_link(
    ...
    let old_dirfd = wasi_ctx
        .get_fd_entry(old_dirfd, wasi::__WASI_RIGHT_PATH_LINK_SOURCE, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
    let new_dirfd = wasi_ctx
        .get_fd_entry(new_dirfd, wasi::__WASI_RIGHT_PATH_LINK_TARGET, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn path_open(
    ...
    let dirfd = wasi_ctx
        ...
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
    let resolved = path_get(dirfd, dirflags, path, oflags & wasi::__WASI_O_CREAT != 0)?;
...
pub(crate) unsafe fn fd_readdir(
    ...
    let fd = dec_fd(fd);
    let file = wasi_ctx
        .get_fd_entry_mut(fd, wasi::__WASI_RIGHT_FD_READDIR, 0)
...
pub(crate) unsafe fn path_readlink(
    ...
    let dirfd = wasi_ctx
        .get_fd_entry(dirfd, wasi::__WASI_RIGHT_PATH_READLINK, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
    let resolved = path_get(dirfd, 0, &path, false)?;
...
pub(crate) unsafe fn path_rename(
    ...
    let old_dirfd = wasi_ctx
        .get_fd_entry(old_dirfd, wasi::__WASI_RIGHT_PATH_RENAME_SOURCE, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
    let new_dirfd = wasi_ctx
        .get_fd_entry(new_dirfd, wasi::__WASI_RIGHT_PATH_RENAME_TARGET, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn fd_filestat_set_times(
    ...
    let fd = dec_fd(fd);
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_FILESTAT_SET_TIMES, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn fd_filestat_set_size(
    ...
    let fd = dec_fd(fd);
    let fd = wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_FD_FILESTAT_SET_SIZE, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn path_filestat_get(
    ...
    let dirfd = wasi_ctx
        .get_fd_entry(dirfd, wasi::__WASI_RIGHT_PATH_FILESTAT_GET, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn path_filestat_set_times(
    ...
    let dirfd = wasi_ctx
        .get_fd_entry(dirfd, wasi::__WASI_RIGHT_PATH_FILESTAT_SET_TIMES, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
    let resolved = path_get(dirfd, dirflags, path, false)?;
...
pub(crate) unsafe fn path_symlink(
    ...
    let dirfd = wasi_ctx
        .get_fd_entry(dirfd, wasi::__WASI_RIGHT_PATH_SYMLINK, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn path_unlink_file(
    ...
    let dirfd = wasi_ctx
        .get_fd_entry(dirfd, wasi::__WASI_RIGHT_PATH_UNLINK_FILE, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
    let resolved = path_get(dirfd, 0, path, false)?;
...
pub(crate) unsafe fn path_remove_directory(
    ...
    let dirfd = wasi_ctx
        .get_fd_entry(dirfd, wasi::__WASI_RIGHT_PATH_REMOVE_DIRECTORY, 0)
        .and_then(|fe| fe.fd_object.descriptor.as_file())?;
...
pub(crate) unsafe fn fd_prestat_get(
    ...
    wasi_ctx
        .get_fd_entry(fd, wasi::__WASI_RIGHT_PATH_OPEN, 0)
        .and_then(|fe| {
            let po_path = fe.preopen_path.as_ref().ok_or(Error::ENOTSUP)?;
            if fe.fd_object.file_type != wasi::__WASI_FILETYPE_DIRECTORY {
